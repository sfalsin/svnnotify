...
def notify(paths, authors):
    ...
    title_string = 'Commits em '+svn_root.split('/')[-2]
    path_string = ', '.join(paths)
    author_string = ', '.join(authors)
    message_string = author_string+'\n'+path_string
...
def discover_changes(last_revision=pysvn.Revision(pysvn.opt_revision_kind.number, 0)):
    ...
    f = open('cache', 'r+')
    f.write(str(last_revision.number))
    f.close        
...
if __name__ == '__main__':
    ...
    with open('cache', 'r+') as f:
        last_revision = pysvn.Revision(pysvn.opt_revision_kind.number,int(f.read()))
    f.closed
    if last_revision =='':
        last_revision = None
    while True:
